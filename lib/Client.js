 ...
 * @property {string} [searchBase] The base tree to search through. Default: `null`.
 * @property {string} [scope] The search scope to use. Can be 'base', 'one',
 * or 'sub'. Default: 'base'.
 * @property {attributes} [array] Default list of attributes to return from
...
const optionsSchema = Joi.object().keys({
  ...
  ldapjs: Joi.object().keys({
    url: Joi.string().uri(['ldap', 'ldaps']).required(),
    searchBase: Joi.string().default(null),
 ...
 * @returns {Promise} Resolves with a value of `true` on success, or rejects
 * with an `Error`.
 */
Client.prototype.authenticate = function authenticate(username, password) {
  ...
  function* generator() {
    ...
    if (username.toLowerCase().startsWith('cn') || username.toLowerCase().startsWith('dn')) {
      ...
      try {
        return yield *auth.call(this, username, password);
      } catch (e) {
        throw e;
      }
    }
 ...
 * supplied in the `options`. Default: `(&(sAMAccountName=username)(objectClass=person))`.
...
Client.prototype.findUser = function findUser(username, options) {
  const filter = (typeof username === 'string') ?
    `(&(sAMAccountName=${username})(objectClass=person))` : null;
...
module.exports = function init($log) {
  ...
  log = ($log) ?
    $log.child({module: 'adlap', clazz: 'Client'}) : pino({level: 'fatal'});
  return Client;
};
